...
with open('file_firm.txt', 'r') as file:
    ...
    if i != 0:
        prof_m = prof / i
        print(f"Средняя прибыль: {prof_m:.2f}")
    else:
        print(f"Следует пересмотреть модель бизнеса. Прибыль отсутствует.")
    pr_m = {"средняя прибыль": round(prof_m)}
    profit.update(pr_m)
    print(f"Прибыль каждой компании: \n {profit}")
with open("file_firm.json", "w") as write_js:
    json.dump(profit, write_js)
    js_str = json.dumps(profit)
    print(f"Итоговый список передан в виде json-объекта со следующим содержимым: \n {js_str}")
